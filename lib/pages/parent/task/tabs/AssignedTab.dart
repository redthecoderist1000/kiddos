...
class _AssignedTabState extends State<AssignedTab> {
  ...
  Future<dynamic> fetchAssignedTasks() async {
    try {
  ...
  unAssignedTask(taskTransId) async {
    try {
      await supabase
          .from('tbl_task_transaction')
          .delete()
          .eq('id', taskTransId);
      setSnackBar('Task was unassigned successfully!', context, Colors.green);
    } catch (e) {
      setSnackBar('Error unassigning task: $e', context, Colors.red);
    }
  }
